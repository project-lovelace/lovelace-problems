...
class Problem2Extended(AbstractProblem):
    ...
    def generate(self):
        ...
        # Generate inputs
        nSoliders = random.randint(minSoliders, maxSoliders)
        nHouses = random.randint(minHouses, maxHouses)
        soliderXPositions = [random.uniform(-xRange, xRange) for _ in range(nSoliders)]
        houseXPositions   = [random.uniform(-xRange, xRange) for _ in range(nHouses)]
    ...
    def solve(self, problem):
        ...
        points = 0
        for s_x in soliders:
            if abs(x_final - s_x) <= blast_radius:
                points = points + 100 - np.floor(np.abs(x_final - s_x))

        for h_x in houses:
            if abs(x_final - h_x) <= blast_radius:
                points = points + 250 - np.floor(np.abs(x_final - h_x))
